...
def test_main(monkeypatch, tmpdir, sessions_dir):
    ...
    monkeypatch.setattr("builtins.input", lambda _: next(inputs))
    monkeypatch.setattr(review_session, "load_sessions", lambda x: [('session_1', {'session_id': 'session_1', 'task_title': 'Task 1', 'task_type': 'test', 'repo_name': 'aider-learning-lab', 'expected_scope': [], 'verdict': None})])
    review_session.main()
